...
import { Send, CheckCircle2, Zap, Users, TrendingUp, Lock, LogOut, Eye, EyeOff, ArrowLeft } from 'lucide-react';
...
export default function AdminPanel() {
    ...
    const [result, setResult] = useState<{ success: boolean; count?: number; total?: number; error?: string } | null>(null);   
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        // Client-side passwords are never compared here. The password is only
        // forwarded to the server, which validates it against a server-side
        // secret before it performs any privileged action.
        if (!password.trim()) {
            setAuthError(true);
        ...
        try {
            const response = await fetch('/api/send-push', {
                ...
                body: JSON.stringify({
                    ...
                    adminPassword: password // Forwarded for server-side validation only
                })
            });

            const data = await response.json();
            setResult({ ...data, success: !!data.success });
            fetchStats();
        } catch (error) {
            ...
            <div className="min-h-screen bg-[#000000] text-slate-200 relative overflow-hidden flex items-center justify-center">
                ...
                <div className="relative z-10 w-full max-w-md px-6">
                    ...
                    <div className="mt-8 flex justify-center">
                        <Link href="/" className="text-slate-500 hover:text-white flex items-center gap-2 transition-colors">
                    ...
                    <div className="mt-8 p-4 bg-white/5 border border-white/10 rounded-xl">
                        <p className="text-xs text-slate-500 text-center font-mono">
                            System Version: 2.1.0 | Encryption: AES-256
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="relative z-10 max-w-7xl mx-auto p-8">
                    ...
                    <div className={`p-6 rounded-2xl mb-8 border-2 ${result.success ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'} animate-fade-in`}>
                        {result.success ? (
                            ...
                            <div>
                                ...
                                {result.error && (
                                    <div className="text-sm text-slate-400 font-mono mt-1">{result.error}</div>
                                )}
                            </div>
                        )}
                    </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-black text-white uppercase tracking-tight">Content Arsenal</h2>
                        <button 
                            onClick={fetchStats}
                            className="text-slate-500 hover:text-white transition-colors flex items-center gap-2 font-mono text-xs uppercase"
                        >
